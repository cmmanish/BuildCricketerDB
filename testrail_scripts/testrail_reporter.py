...
class TestRailReporter (object):
    ...
    def __init__(self):
        ...
        config_path = os.path.join("/Users/", uname, ".testrailreporter.json")
        with open(config_path, 'r') as fh:
            contents = fh.read()
        user_config = json.loads(contents)

        # Init interface to the TestRail system
        self.testrail = testrail_api.TestRailApi(user_config.get("TR_Base_URL"),
                                                 user_config.get("TR_User_Name"),
                                                 user_config.get("TR_User_PW"))
...
if __name__ == '__main__':
    ...
    if (len(args) > 1):
        print "Please specify a single input file path"
        sys.exit(-1)
